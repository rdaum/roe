...
impl WindowNode {
    ...
    pub fn new_split(direction: SplitDirection, ratio: f32, first: WindowNode, second: WindowNode) -> Self {
        WindowNode::Split {
            direction,
            ratio,
            first: Box::new(first),
...
impl Editor {
    ...
    pub fn calculate_window_layout(&mut self) {
        ...
        self.layout_node(&self.window_tree.clone(), 0, 0, available_width, available_height);
    }
    ...
    pub fn split_horizontal(&mut self) -> WindowId {
        ...
        self.window_tree = self.split_node_horizontal(&self.window_tree.clone(), self.active_window, new_window_id);
    ...
    pub fn split_vertical(&mut self) -> WindowId {
        ...
        // Update the tree structure
        self.window_tree = self.split_node_vertical(&self.window_tree.clone(), self.active_window, new_window_id);
        self.calculate_window_layout();
        new_window_id
    }
    
    /// Split a node horizontally in the tree
    fn split_node_horizontal(&self, node: &WindowNode, target_window: WindowId, new_window: WindowId) -> WindowNode {
    ...
    fn split_node_horizontal_impl(node: &WindowNode, target_window: WindowId, new_window: WindowId) -> WindowNode {
        match node {
            WindowNode::Leaf { window_id } => {
            ...
            WindowNode::Split { direction, ratio, first, second } => {
                ...
                let new_second = Self::split_node_horizontal_impl(second, target_window, new_window);
    ...
    fn split_node_vertical(&self, node: &WindowNode, target_window: WindowId, new_window: WindowId) -> WindowNode {
        Self::split_node_vertical_impl(node, target_window, new_window)
    }
    
    /// Implementation of split_node_vertical that doesn't use self
    fn split_node_vertical_impl(node: &WindowNode, target_window: WindowId, new_window: WindowId) -> WindowNode {
        match node {
            WindowNode::Leaf { window_id } => {
                if *window_id == target_window {
    ...
    pub fn switch_window(&mut self) -> WindowId {
        ...
        let current_index = window_ids.iter().position(|&id| id == self.active_window).unwrap_or(0);
    ...
    pub fn delete_window(&mut self) -> bool {
        ...
        let (new_tree, deleted, suggested_active) = self.delete_node_from_tree_with_selection(&self.window_tree.clone(), self.active_window);
    ...
    pub fn delete_other_windows(&mut self) -> bool {
        ...
        let other_windows: Vec<WindowId> = self.windows.keys()
    ...
    fn delete_node_from_tree_with_selection(&self, node: &WindowNode, target_window: WindowId) -> (WindowNode, bool, Option<WindowId>) {
        match node {
            WindowNode::Leaf { window_id } => {
                if *window_id == target_window {
            ...
            WindowNode::Split { direction, ratio, first, second } => {
                ...
                let (new_first, first_deleted, first_suggestion) = self.delete_node_from_tree_with_selection(first, target_window);
                let (new_second, second_deleted, second_suggestion) = self.delete_node_from_tree_with_selection(second, target_window);
                ...
                if first_deleted && second_deleted {
                    ...
                    match &**first {
                        ...
                        _ => {
                            ...
                            (WindowNode::new_split(*direction, *ratio, new_first, new_second), true, first_suggestion)
                        }
                    }
                    ...
                    match &**second {
                        ...
                        _ => {
                            ...
                            (WindowNode::new_split(*direction, *ratio, new_first, new_second), true, second_suggestion)
                        }
                    }
                } else {
                    // No deletion in this subtree, reconstruct with possibly updated children
                    (WindowNode::new_split(*direction, *ratio, new_first, new_second), false, None)
                }
            }
        }
    }
...
mod tests {
    ...
    fn test_cursor_move_right() {
        ...
        assert!(actions.iter().any(|action| matches!(action, ChromeAction::CursorMove(_))));
    ...
    fn test_cursor_position_calculation() {
        ...
        if let Some(ChromeAction::CursorMove((x, y))) = actions.iter().find(|a| matches!(a, ChromeAction::CursorMove(_))) {
    ...
    fn test_spatial_window_switching() {
        ...
        let w1_pos = (editor.windows[w1].x, editor.windows[w1].y);
        let w2_pos = (editor.windows[w2].x, editor.windows[w2].y);
        let w3_pos = (editor.windows[w3].x, editor.windows[w3].y);
        ...
        let positions: Vec<(u16, u16)> = spatial_order.iter()
    ...
    fn test_spatial_order_with_complex_layout() {
        ...
        let w5 = editor.split_vertical(); // w3 left, w5 right in top-right
        
        // Split w2 vertically to create w4  
        editor.active_window = w2;
        let w4 = editor.split_vertical(); // w2 left, w4 right in bottom half
        ...
        // Print debug info to understand the layout
        println!("Window positions:");
        for &id in &spatial_order {
            let window = &editor.windows[id];
            println!("  Window {:?}: ({}, {})", id, window.x, window.y);
        }
    ...
    fn verify_window_tree_integrity(editor: &Editor) {
        ...
        for tree_window in &tree_windows {
            assert!(editor.windows.contains_key(*tree_window), 
        ...
        for window_id in remaining_windows {
            assert!(tree_windows.contains(&window_id),
                   "Window {window_id:?} exists in SlotMap but not in tree");
        }
        
        // Active window should exist in both
        assert!(editor.windows.contains_key(editor.active_window), 
               "Active window {:?} not in SlotMap", editor.active_window);
        assert!(tree_windows.contains(&editor.active_window),
               "Active window {:?} not in tree", editor.active_window);
    }
    ...
    fn extract_windows_recursive(node: &WindowNode, windows: &mut std::collections::HashSet<WindowId>) {
        match node {
            WindowNode::Leaf { window_id } => {
