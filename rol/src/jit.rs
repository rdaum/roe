...
impl VarJIT {
    ...
    pub fn create_binary_function(&mut self, name: &str) -> FuncId {
        ...
        self.module.declare_function(name, Linkage::Export, &sig).unwrap()
    }
...
impl VarBuilder {
    ...
    pub fn is_int(&self, builder: &mut FunctionBuilder, var: Value) -> Value {
        ...
        let min_num_high = builder.ins().iconst(types::I64, (MIN_NUMBER & HIGH16_TAG) as i64);
    ...
    pub fn is_number(&self, builder: &mut FunctionBuilder, var: Value) -> Value {
        ...
        let is_ge_min = builder.ins().icmp(IntCC::UnsignedGreaterThanOrEqual, var, min_number);
        
        // Check for symbol (high bits match SYMBOL_TAG pattern)
        let symbol_high = builder.ins().iconst(types::I64, (SYMBOL_TAG & HIGH16_TAG) as i64);
        let high_mask = builder.ins().iconst(types::I64, HIGH16_TAG as i64);
    ...
    pub fn is_list(&self, builder: &mut FunctionBuilder, var: Value) -> Value {
        ...
        let is_ge_min = builder.ins().icmp(IntCC::UnsignedGreaterThanOrEqual, var, min_ptr);
    ...
    pub fn extract_double(&self, builder: &mut FunctionBuilder, var: Value) -> Value {
        ...
        let offset = builder.ins().iconst(types::I64, DOUBLE_ENCODE_OFFSET as i64);
    ...
    pub fn emit_double_equals(&self, builder: &mut FunctionBuilder, lhs: Value, rhs: Value) -> Value {
        let a = self.extract_double(builder, lhs);
        let b = self.extract_double(builder, rhs);
...
mod tests {
    ...
    fn test_round_trip_construction() {
        ...
        let code_ptr = jit.compile_function(func_id, |builder, var_builder| {
            ...
            builder.ins().brif(is_zero, int_block, &[], check_double_block, &[]);
            ...
            builder.ins().brif(is_one, double_block, &[], check_bool_block, &[]);
            ...
            builder.ins().brif(is_two, bool_block, &[], default_block, &[]);
    ...
    fn test_jit_symbol_operations() {
        ...
        let code_ptr = jit.compile_function(func_id, |builder, _var_builder| {
            ...
            let symbol_tag = builder.ins().iconst(types::I64, crate::var::SYMBOL_TAG as i64);
    ...
    fn test_jit_comprehensive_type_checking() {
        ...
        let code_ptr = jit.compile_function(func_id, |builder, var_builder| {
            ...
            builder.ins().brif(is_zero, int_block, &[], check_double, &[]);
            ...
            builder.ins().brif(is_one, double_block, &[], check_bool, &[]);
    ...
    fn test_jit_comprehensive_heap_type_checking() {
        ...
        let code_ptr = jit.compile_function(func_id, |builder, var_builder| {
            ...
            builder.ins().brif(is_three, list_block, &[], check_string, &[]);
            ...
            builder.ins().brif(is_four, string_block, &[], default_block, &[]);
