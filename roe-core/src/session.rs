    ...
    async fn apply_mica_events(
        ...
        for action in events.host_actions {
            match action.name.as_str() {
                "agent_render" => {
                    let Some(text) = action.text else {
                        lifecycle.push(LifecycleEvent::Error(
                            "Mica agent rendering lost its text".to_owned(),
                        ));
                        continue;
                    };
                    if text.chars().count() > MAX_AGENT_BUFFER_CHARS {
                        lifecycle.push(LifecycleEvent::Overloaded {
                            detail: format!(
                                "agent transcript display exceeds the {MAX_AGENT_BUFFER_CHARS}-character limit"
                            ),
                        });
                        continue;
                    }
                    if action.phase.as_deref() == Some("update") {
                        let Some(buffer_id) = action.buffer else {
                            // A killed buffer may still have an in-flight update. Never
                            // recreate it or redirect the update to another buffer.
                            continue;
                        };
                        let Some(buffer) = self.editor.buffers.get(buffer_id) else {
                            continue;
                        };
                        if buffer.kind() != crate::buffer::BufferKind::Results
                            || !buffer.is_read_only()
                        {
                            lifecycle.push(LifecycleEvent::Error(
                                "Mica agent update requires a read-only results buffer".to_owned(),
                            ));
                            continue;
                        }
                        ...
                        if buffer.text_revision() != revision {
                            for (window_id, window) in &mut self.editor.windows {
                                if window.active_buffer == buffer_id {
                                    ...
                                    invalidations
                                        .push(Invalidation::View(self.view_ids[&window_id]));
                                }
                            }
                        }
                        continue;
                    }
                    let (Some(view), Some(name), Some("open")) =
                        (action.view, action.buffer_name, action.phase.as_deref())
                    else {
                        lifecycle.push(LifecycleEvent::Error(
                            "Mica agent display lost its view, name, or phase".to_owned(),
                        ));
                        continue;
                    };
                    if name.chars().count() > MAX_BUFFER_NAME_CHARS
                        || !self.editor.windows.contains_key(view)
                    {
                        lifecycle.push(LifecycleEvent::Error(
                            "Mica agent display has an invalid name or stale view".to_owned(),
                        ));
                        continue;
                    }
